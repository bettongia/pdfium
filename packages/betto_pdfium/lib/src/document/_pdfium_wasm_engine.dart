...
// Worker-reusable PDFium marshalling engine.
//
// This file contains every PDFium call/marshalling function used by the web
// (WASM) backend, extracted so it can be invoked identically from two
// contexts:
//   1. The main thread, directly — used by `test/pdf_document_web_test.dart`
//      (via `_document_web.dart`'s pre-worker-offload behaviour) and by the
//      Phase 4 direct-engine coverage tests, which bypass the Worker entirely
//      to keep this logic visible to `dart test -p chrome --coverage`.
//   2. Inside the dedicated PDFium Worker (`_pdfium_worker_entry.dart`), which
//      owns the WASM module + document registry after the Phase 3 rewrite of
//      `PdfDocumentImpl` into a thin RPC client.
//
// Every function here takes an explicit [PdfiumModule] (and, where relevant,
// document/page pointers or a registry record) as a parameter — there are no
// static, main-thread-only globals in this file. This is what makes the logic
// safely callable from a Worker's own global scope.
//
// None of the functions in this file know about the `Worker`/`postMessage`
// RPC protocol, document tokens, or `Finalizer` cleanup — those concerns stay
// in `_document_web.dart` (the main-thread RPC client) and
// `_pdfium_worker_entry.dart` (the worker-side dispatch shell) respectively.
...
///
/// This function is callable directly on the main thread — used today by
/// `_document_web.dart` (pre-Worker-offload) and by the Phase 4 direct-engine
/// coverage tests, which deliberately bypass the Worker to keep this bootstrap
/// path visible to `dart test -p chrome --coverage`. The Worker entry point
/// (`_pdfium_worker_entry.dart`) uses a different bootstrap
/// (`importScripts('pdfium.js')`) because DOM APIs — including
/// `document.createElement` — are unavailable in worker scope.
///
/// Throws [PdfiumException] if pdfium.js/pdfium.wasm fail to load or
/// initialise within 30 seconds.
Future<PdfiumModule> loadPdfiumModule() async {
...
/// Loads a PDF document from raw [bytes] and returns the resulting document
/// registry record (docPtr + bufPtr).
///
/// Allocates a WASM heap buffer, copies [bytes] into it, and calls
/// `FPDF_LoadMemDocument64`. The returned `bufPtr` must remain allocated
/// (i.e. not freed) until [engineCloseDocument] is called — PDFium does not
/// copy the buffer.
///
/// Throws [PdfiumException] if the WASM heap allocation fails.
/// Throws [PdfExtractionException] if the document is invalid or
/// password-protected.
({int docPtr, int bufPtr}) engineLoadDocument(
  PdfiumModule module,
  Uint8List bytes,
) {
  ...
  return (docPtr: docPtr, bufPtr: bufPtr);
}

/// Releases the PDFium document handle at [docPtr] and frees the WASM heap
/// buffer at [bufPtr].
void engineCloseDocument(PdfiumModule module, int docPtr, int bufPtr) {
  module.fpdfCloseDocument(docPtr);
  module.free(bufPtr);
}

/// Returns the total number of pages in the document at [docPtr].
int enginePageCount(PdfiumModule module, int docPtr) =>
    module.fpdfGetPageCount(docPtr);

// =============================================================================
// Metadata, document info, page size
// =============================================================================

/// Returns the Info dictionary metadata for the document at [docPtr].
///
...
PdfMetadata engineGetMetadata(PdfiumModule module, int docPtr) {
  return PdfMetadata(
    ...
    modDate: PdfDateParser.parse(_readMetaTextField(module, docPtr, 'ModDate')),
  );
}

/// Returns file-level information for the document at [docPtr].
///
...
PdfDocumentInfo engineGetDocumentInfo(PdfiumModule module, int docPtr) {
...
/// Throws [PdfExtractionException] if the page fails to load.
PdfPageSize engineGetPageSize(PdfiumModule module, int docPtr, int pageIndex) {
...
// =============================================================================
// Page-index resolution (shared orchestration for streaming operations)
// =============================================================================

/// Resolves the list of page indices to iterate for a streaming operation.
///
/// When [pageIndex] is null, returns all page indices `0..count-1`. When
/// [pageIndex] is specified, returns a single-element list containing it.
///
/// Throws [RangeError] if [pageIndex] is out of range.
List<int> engineResolvePageIndices(
  PdfiumModule module,
  int docPtr,
  int? pageIndex,
) {
  ...
  return pageIndex != null ? [pageIndex] : List.generate(count, (i) => i);
}
...
/// Renders page [pageIndex] of the document at [docPtr] to a BGRA pixel
/// buffer.
///
...
({Uint8List pixels, int pixelWidth, int pixelHeight}) engineRenderPageToBytes(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
  int pixelWidth,
  ...
  try {
    ...
    try {
      ...
      return (pixels: pixels, pixelWidth: pixelWidth, pixelHeight: pixelHeight);
    } finally {
...
/// Returns the thumbnail for page [pageIndex] of the document at [docPtr].
///
...
PdfThumbnail? engineGetThumbnail(
  PdfiumModule module,
  int docPtr,
  int pageIndex, {
  ...
  final size = engineGetPageSize(module, docPtr, pageIndex);
  ...
  final rendered = engineRenderPageToBytes(module, docPtr, pageIndex, pw, ph);
  return PdfThumbnail(
...
PdfPageText engineExtractPageText(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
) {
...
List<PdfAnnotation> engineExtractPageAnnotations(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
) {
...
List<PdfImage> engineExtractPageImages(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
  bool includeBitmap,
...
/// [pageIndex] of the document at [docPtr], or null when the object has no
/// renderable bitmap.
///
/// Throws [RangeError] if [pageIndex] is out of range or [objectIndex] is
/// negative.
PdfImageBitmap? engineRenderImage(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
  int objectIndex,
...
///
/// [flagsMask] is a bitmask: FPDF_MATCHCASE=0x01, FPDF_MATCHWHOLEWORD=0x02,
/// FPDF_CONSECUTIVE=0x04.
List<PdfSearchMatch> engineSearchPage(
  PdfiumModule module,
  int docPtr,
  int pageIndex,
  String query,
...
List<PdfTocEntry> engineTableOfContents(PdfiumModule module, int docPtr) {
  final visited = <int>{};
