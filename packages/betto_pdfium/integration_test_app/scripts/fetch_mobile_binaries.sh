...
#   ../betto_pdfium_ios/ios/Frameworks/pdfium.xcframework/  (unzipped from .xcframework.zip)
...
if [ "$ANDROID_ONLY" -eq 0 ]; then
    ...
    IOS_DEST_DIR="$(dirname "$(dirname "$APP_DIR")")/betto_pdfium_ios/ios/Frameworks"
    ...
    echo "  iOS xcframework installed at betto_pdfium_ios/ios/Frameworks/pdfium.xcframework"
fi
...
if [ "$ANDROID_ONLY" -eq 0 ]; then
    ...
    echo "  2. Run 'flutter pub get' from integration_test_app/ to let Flutter"
    echo "       discover the pdfium_ios plugin automatically."
    echo "  3. Run 'make ios_test' or open ios/Runner.xcworkspace in Xcode to build."
fi
