...
echo "staging WASM artifacts to $PDFIUM_DIST/$PDFIUM_PLATFORM/ ..."
mkdir -p $PDFIUM_DIST/$PDFIUM_PLATFORM

# Emscripten produces a .wasm module and a .js glue file.
cp $PDFIUM_OUT/*.wasm $PDFIUM_DIST/$PDFIUM_PLATFORM/
cp $PDFIUM_OUT/*.js   $PDFIUM_DIST/$PDFIUM_PLATFORM/ 2>/dev/null || true
