 ...
 *
 * __attribute__((used)): instructs the compiler to retain this variable even
 * if it appears unused at the C level, preventing dead-code elimination
 * before the linker runs.
 *
 * static: limits linkage to this translation unit; the anchor itself need
 * not be visible to other compilation units.
 */
